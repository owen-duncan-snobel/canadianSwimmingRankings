...
class ReactTable extends Component {
	render() {
		...
		try {
					...
					<thead>
						<tr>
							{this.props.allowedKeys.map((key) => {
								return (
									<th
										key={JSON_ATTRIBUTES[key]}
										name={JSON_ATTRIBUTES[key]}
									>
										{JSON_ATTRIBUTES[key]}
									</th>
								);
						...
						{this.props.tableData.map((item) => {
							return (
								<tr key={Object.entries(item).toString()}>
									{Object.entries(item)
										.filter(([key, value]) =>
											this.props.allowedKeys.includes(key)
										)
										.map(([key, value]) => {
											return (
												<td key={key + value}>
			...
			console.log(
				'Error: Unable to create a table with data. Check to see if data passed in null'
			);
			return <div></div>;
		}
	}
