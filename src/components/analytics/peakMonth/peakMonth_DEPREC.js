...
class PeakMonth extends Component {
	render() {
		...
		let allSwimmerDataSubComponents = this.props
				...
				callbacks: {
					...
					label: (tooltipItem, d) => {
						...
						if (tooltipItem.datasetIndex === 1) {
							labelArr.push(
								monthPercent[tooltipItem.index] + '%'
							);
						} else {
							labelArr.push(
								'PLACE         SWIMMER                  TIME'
					...
					<Col className="mt-1" lg={9} xs={12}>
						<div>
							<h6 className="text-center">
								{this.props.event + ': Month of Best Time'}{' '}
							</h6>
						</div>
						<Bar
							name="Selected Events Best time over months chart"
							data={event}
			...
			selectedEvents = (
				<Row className="justify-content-md-center">
					<Col className="mt-1" md={10} xs={12}>
						...
						<Bar
							name="All Events Best time over months chart"
							data={allEvents}
...
PeakMonth.propTypes = {
	 ...
	 */
	allSwimmerDataSubComponents: PropTypes.arrayOf(
